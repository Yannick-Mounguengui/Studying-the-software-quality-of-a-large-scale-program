...
public final class ReflectiveTypeAdapterFactory implements TypeAdapterFactory {
  ...
  public <T> TypeAdapter<T> create(Gson gson, final TypeToken<T> type) {
    ...
    FilterResult filterResult =
        ReflectionHelper.getFilterResult(reflectionFilters, raw);
  ...
  private static <M extends AccessibleObject & Member> void checkAccessible(Object object, M member) {
    if (!ReflectionHelper.canAccess(member, Modifier.isStatic(member.getModifiers()) ? null : object)) {
  ...
  private Map<String, BoundField> getBoundFields(Gson context, TypeToken<?> type, Class<?> raw,
    ...
    while (raw != Object.class) {
      ...
      if (raw != originalRaw && fields.length > 0) {
        FilterResult filterResult = ReflectionHelper.getFilterResult(reflectionFilters, raw);
        if (filterResult == FilterResult.BLOCK_ALL) {
